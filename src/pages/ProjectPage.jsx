...
function AgentAvatar({ agent }) {
  return <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-50 to-violet-100 text-xl ring-1 ring-white">{agent.emoji}</span>;
}
...
function ProjectList({ projects, onOpen, onCreate }) {
  ...
  return <main className="flex min-h-[calc(100vh-56px)] flex-col px-8 pb-32 pt-6">
    <div className="flex flex-wrap items-end gap-3"><div className="mr-auto"><h1 className="text-2xl font-semibold">项目空间</h1><p className="mt-1 text-sm text-neutral-400">围绕共享说明、群聊、Agent 和资料组织协作。</p></div><div className="relative"><i className="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索项目" className="w-72 pl-9" /></div><Button onClick={onCreate}><i className="ri-add-line" />创建项目</Button></div>
    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <button onClick={onCreate} className="flex min-h-[220px] flex-col items-center justify-center rounded-3xl border-2 border-dashed border-neutral-200 bg-white/30 text-neutral-400 transition hover:border-orange-300 hover:bg-orange-50/30 hover:text-orange-500"><i className="ri-add-line text-4xl" /><span className="mt-3 text-sm font-medium">创建项目空间</span></button>
      {filtered.map(project => <button key={project.id} onClick={() => onOpen(project.id)} className="rounded-3xl bg-white p-6 text-left ring-1 ring-neutral-200/70 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/5"><div className="flex items-start justify-between"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-100 text-2xl text-orange-500"><i className="ri-folder-3-fill" /></span><span className="rounded-full bg-neutral-100 px-2.5 py-1 text-[11px] text-neutral-500">{project.scope}</span></div><h2 className="mt-5 text-lg font-semibold">{project.name}</h2><p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-neutral-400">{project.description || '尚未填写项目说明'}</p><div className="mt-5 flex items-center gap-4 border-t border-neutral-100 pt-4 text-xs text-neutral-400"><span><i className="ri-robot-2-line" /> {project.agentIds.length} Agent</span><span><i className="ri-group-line" /> {project.members.length} 成员</span><span><i className="ri-message-3-line" /> {project.sessions.length} 群聊</span></div></button>)}
    </div>
  </main>;
}
...
function Overview({ project, agents, onTab, onUpdateDescription }) {
  ...
  return <div className="flex-1 overflow-y-auto p-6"><div className="mx-auto max-w-[1120px]">
...
function ProjectChat({ project, onUpdate }) {
  ...
  return <div className="flex min-h-0 flex-1">
    <aside className="w-64 shrink-0 border-r border-neutral-200/60 bg-white/45 p-4"><div className="flex items-center justify-between px-2"><span className="text-xs font-semibold text-neutral-500">项目群聊</span><button className="text-orange-500"><i className="ri-add-line" /></button></div><div className="mt-3 space-y-1">{project.sessions.map(session => <button key={session.id} onClick={() => setActiveId(session.id)} className={`w-full rounded-xl px-3 py-3 text-left ${active?.id === session.id ? 'bg-cyan-50 text-cyan-700' : 'hover:bg-white'}`}><div className="flex items-center gap-2"><i className="ri-group-2-line" /><span className="truncate text-sm font-medium">{session.name}</span></div><div className="mt-1 truncate pl-6 text-[10px] text-neutral-400">{session.participants.length} 位参与者</div></button>)}</div></aside>
    <section className="flex min-w-0 flex-1 flex-col bg-white/50">
      <header className="border-b border-neutral-200/60 bg-white/80 px-6 py-4"><div className="font-semibold">{active?.name}</div><div className="mt-1 truncate text-xs text-neutral-400">{active?.participants.join('、')}</div></header>
...
export default function ProjectPage({ onNavigate }) {
  ...
  return <PageShell>
    ...
    {!project ? <ProjectList projects={projects} onOpen={id => { setActiveId(id); setTab('overview'); }} onCreate={() => setCreateOpen(true)} /> : <main className="flex min-h-[calc(100vh-56px)] flex-col pb-24">
      <header className="glass-soft flex shrink-0 items-center gap-4 border-x-0 border-t-0 px-6 py-3"><button onClick={() => setActiveId(null)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/70 text-neutral-500 ring-1 ring-neutral-200/60"><i className="ri-arrow-left-line" /></button><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-100 text-xl text-orange-500"><i className="ri-folder-3-fill" /></span><div className="min-w-0"><h1 className="truncate font-semibold">{project.name}</h1><div className="mt-0.5 text-[11px] text-neutral-400">{project.scope} · {project.owner} 负责</div></div><div className="ml-auto flex -space-x-2">{projectAgents.slice(0, 3).map(agent => <AgentAvatar key={agent.id} agent={agent} />)}</div></header>
      <div className="flex min-h-0 flex-1">
        <aside className="glass-soft w-48 shrink-0 border-y-0 border-l-0 p-3"><nav className="space-y-1">{spaceTabs.map(([id, label, icon]) => <button key={id} onClick={() => setTab(id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${tab === id ? 'bg-orange-500 font-medium text-white shadow-sm' : 'text-neutral-500 hover:bg-white hover:text-neutral-800'}`}><i className={`${icon} text-lg`} />{label}</button>)}</nav><div className="mt-5 border-t border-neutral-200/60 pt-4"><div className="px-3 text-[10px] font-medium uppercase tracking-wider text-neutral-400">空间边界</div><p className="mt-2 px-3 text-[11px] leading-5 text-neutral-400">Agent 仅使用本项目授权的资料和 Tools。</p></div></aside>
